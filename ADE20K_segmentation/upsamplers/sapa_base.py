...
class SAPA(nn.Module):
    def __init__(self, dim_y, dim_x=None, scale_factor=2, up_kernel_size=5, embedding_dim=32,
    ...
    def forward(self, y, x):
        ...
        return self.attention(q.contiguous(), k.contiguous(), x.contiguous())

    def attention(self, q, k, v):
        attn = F.softmax(qk(q, k, self.up_kernel_size, self.scale_factor), dim=1)
        return av(attn, v, self.up_kernel_size, self.scale_factor)
