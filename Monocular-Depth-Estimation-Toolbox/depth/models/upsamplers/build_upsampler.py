...
from .indexnet import IndexUp
from .a2u import A2UUp
from .rrfu import RRFU


def build_upsampler(cfg, in_channels, scale_factor, y_channel=None):
    ...
    if upsample_mode == 'nearest':
        ...
        fade_cfg = dict(up_kernel_size=5,
                        gating=False)
        fade_cfg.update(cfg)
        return FADE(y_channel,
                    in_channels_de=in_channels,
                    scale_factor=scale_factor,
    ...
    elif upsample_mode == 'indexnet':
        return IndexUp(in_channels=y_channel, scale_factor=scale_factor)
    elif upsample_mode == 'a2u':
        return A2UUp(in_channels=y_channel, scale=scale_factor)
    ...
    elif upsample_mode == 'rrfu':
        if cfg['guided']:
            return RRFU(in_channels,
                            scale_factor=scale_factor, guided=True)
        else:
            return RRFU(in_channels,
                            scale_factor=scale_factor)
    else:
