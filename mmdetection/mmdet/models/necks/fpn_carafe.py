...
class FPN_CARAFE(BaseModule):
    ...
    def forward(self, inputs):
        ...
        vis_whether = True  #可视化开关
        if vis_whether:
            self.record += 1
            featureVisualizer = FeatureVisualizer('/newHome/S6_XJ/mmdetection/work_dirs/vis_results')
        ...
        for i, lateral_conv in enumerate(self.lateral_convs):
            ...
            if vis_whether:
                featureVisualizer.save_features(input, self.record, 'x'+str(i)+'laterals')
                featureVisualizer.save_features(lateral, self.record, 'x'+str(i)+'lateral_conv')
        ...
        for i in range(len(laterals) - 1, 0, -1):
            ...
            if vis_whether:
                featureVisualizer.save_features(upsample_feat, self.record, 'x'+str(i)+'laterals_upsampled')
                featureVisualizer.save_features(laterals[i - 1], self.record, 'x'+str(i)+'lateral_fusion')
        ...
        for i in range(num_conv_outs):
            ...
            if vis_whether:
                featureVisualizer.save_features(out, self.record, 'x'+str(i)+'lateral_融合卷积后')
          
        return tuple(outs)

@MODELS.register_module()
class FPN_CARAFE_Upsample(BaseModule):
    """FPN_CARAFE is a more flexible implementation of FPN. It allows more
    choice for upsample methods during the top-down pathway.
    ...
    def __init__(self,
        ...
        assert init_cfg is None, 'To prevent abnormal initialization ' \
                                 'behavior, init_cfg is not allowed to be set'
        super(FPN_CARAFE_Upsample, self).__init__(init_cfg)
        assert isinstance(in_channels, list)
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.num_ins = len(in_channels)
        ...
        self.upsample_mode = upsample_cfg['type']
        self.relu = nn.ReLU(inplace=False)
        ...
        for i in range(self.start_level, self.backbone_end_level):
            ...
            self.fpn_convs.append(fpn_conv)
            if i != self.backbone_end_level - 1:
                self.upsample_modules.append(build_upsampler(upsample_cfg, out_channels, scale_factor=2))
    ...
    def init_weights(self):
        """Initialize the weights of module."""
        super(FPN_CARAFE_Upsample, self).init_weights()
        for m in self.modules():
            if isinstance(m, (nn.Conv2d, nn.ConvTranspose2d)):
                xavier_init(m, distribution='uniform')
        for m in self.modules():
    ...
    def forward(self, inputs):
        ...
        for i in range(len(laterals) - 1, 0, -1):
            if self.guided_upsample:
                if self.upsample_mode == 'rrfu':
                    upsample_feat = self.upsample_modules[i - 1](laterals[i], laterals[i - 1])
                else:
                    upsample_feat = self.upsample_modules[i - 1](laterals[i - 1], laterals[i])
            else:
                upsample_feat = self.upsample_modules[i - 1](laterals[i])
            laterals[i - 1] = self.tensor_add(laterals[i - 1], upsample_feat)
