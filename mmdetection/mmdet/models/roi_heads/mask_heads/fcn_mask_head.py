...
from mmdet.models.upsamplers import build_upsampler
...
class FCNMaskHead(BaseModule):
    ...
    def __init__(self,
        ...
        if self.upsample_cfg['type'] not in [
                None, 'deconv', 'nearest', 'bilinear', 'carafe', 'defsampler'
        ]:
            raise ValueError(
        ...
        if self.upsample_method is None:
            ...
            self.upsample = build_upsample_layer(upsample_cfg_)
        elif self.upsample_method == 'defsampler':
            upsample_cfg_.update(
                channels=upsample_in_channels, scale_factor=self.scale_factor)
            self.upsample = build_upsampler(upsample_cfg_, upsample_in_channels, scale_factor=2)
        else:
            # suppress warnings
            align_corners = (None
