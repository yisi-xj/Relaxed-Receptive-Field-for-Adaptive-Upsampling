...
class LightHamHead_Upsample(BaseDecodeHead):
    ...
    def __init__(self, ham_channels=512, ham_kwargs=dict(), upsample_cfg=dict(type='defsampler'), **kwargs):
        ...
        for idx in range(1, len(self.in_channels)):
            scale_factor = 2 ** idx  # 计算总上采样倍数
            self.upsample_modules.append(
                build_upsampler(upsample_cfg.copy(), in_channels=self.in_channels[idx], scale_factor=scale_factor)
            )
    ...
    def forward(self, inputs):
        ...
        for i, x in enumerate(inputs):
            if i == 0:
                ...
                continue
            # 每个输入层i应用对应的上采样模块一次
            if (i-1) < len(self.upsample_modules):
                x = self.upsample_modules[i-1](x)  # 索引调整为i-1
            # 确保尺寸匹配
