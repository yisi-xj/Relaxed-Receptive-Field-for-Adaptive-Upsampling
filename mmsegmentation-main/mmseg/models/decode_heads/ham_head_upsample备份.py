...
class LightHamHead_Upsample(BaseDecodeHead):
    """SegNeXt decode head.
    ...
    def __init__(self, ham_channels=512, ham_kwargs=dict(), upsample_cfg=dict(type='defsampler'), **kwargs):
        ...
        for idx in range(1, len(self.in_channels)):
            ...
            for _ in range(num_upsamples):
                self.upsample_modules.append(
                    build_upsampler(upsample_cfg.copy(), in_channels=self.in_channels[idx], scale_factor=2)
                )
    ...
    def forward(self, inputs):
        ...
        for i, x in enumerate(inputs):
            if i == 0:
                upsampled.append(x)
                continue
            # Apply i times upsampling
            for _ in range(i):
                if module_idx < len(self.upsample_modules):
                    x = self.upsample_modules[module_idx](x)
                    module_idx += 1
            # Ensure size matches target
            if x.shape[-2:] != target_size:
                print('尺寸不匹配', x.shape, target_size)
            upsampled.append(x)

        inputs = torch.cat(upsampled, dim=1)
